    ...
    /*
     for (i = 0; i < 10; i++)
      ...
      if( byte[0]=='P' && (byte[1]=='3' || byte[1]=='6') )
        ...
        else if ( byte[0]==0xff && byte[1]==0xd8 && byte[2]==0xff && byte[3]==0xe0 &&
     strncmp(&byte[6], "JFIF", 4)) return PIC_JPEG_FILE; else return PIC_UNKNOWN_FILE;
    */
}
...
Pic_file_format pic_filename_type(char* file)
{
    ...
    if (!strcmp(suff, ".tiff") || !strcmp(suff, ".tif"))
        return PIC_TIFF_FILE;
    if (!strcmp(suff, ".ppm"))
        return PIC_PPM_FILE;
    return PIC_UNKNOWN_FILE;
