...
void Vk_Jello::DestroyDebugUtilsMessengerEXT(
...
void Vk_Jello::particlePosUpdate()
{
    updateJelloVertexBuffers();
    void* data;
    vkMapMemory(device, m_jelloVertexBufferMemory, 0,
                sizeof(m_jelloVertices[0]) * m_jelloVertices.size(), 0, &data);
    memcpy(data, m_jelloVertices.data(),
           sizeof(m_jelloVertices[0]) * m_jelloVertices.size());
    vkUnmapMemory(device, m_jelloVertexBufferMemory);
}

void Vk_Jello::drawFrame()
{
...
void Vk_Jello::cleanupSwapChain()
{
...
void Vk_Jello::destroyDepthBuffers()
{
    for (auto imageView : depthBufferImageViews)
    {
        vkDestroyImageView(device, imageView, nullptr);
    }

    for (auto image : depthBufferImages)
    {
        vkDestroyImage(device, image, nullptr);
    }

    for (auto memory : depthBufferMemories)
    {
        vkFreeMemory(device, memory, nullptr);
    }
}

void Vk_Jello::cleanup()
{
    cleanupSwapChain();
    destroyDepthBuffers();
    ...
    vkDestroyBuffer(device, m_jelloVertexBuffer, nullptr);
    vkFreeMemory(device, m_jelloVertexBufferMemory, nullptr);

    vkDestroyBuffer(device, m_jelloIndexBuffer, nullptr);
    vkFreeMemory(device, m_jelloIndexBufferMemory, nullptr);
...
void Vk_Jello::populateDebugMessengerCreateInfo(
...
void Vk_Jello::pickPhysicalDevice()
{
...
void Vk_Jello::createLogicalDevice()
{
...
void Vk_Jello::createDepthBuffers()
{
    assert(swapChainImages.size() != 0);
    depthBufferImages.resize(swapChainImages.size());
    depthBufferMemories.resize(swapChainImages.size());
    depthBufferImageViews.resize(swapChainImages.size());

    for (size_t i = 0; i < depthBufferImageViews.size(); i++)
    {
        ...
        imageCreateInfo.format = depthBufferFormat;
        ...
        if (vkCreateImage(device, &imageCreateInfo, nullptr, &depthBufferImages[i]) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to create depth buffer images!");
        }

        VkMemoryRequirements memRequirements;
        vkGetImageMemoryRequirements(device, depthBufferImages[i], &memRequirements);
        ...
        if (vkAllocateMemory(device, &allocInfo, nullptr, &depthBufferMemories[i]) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to allocate depth image memory!");
        }

        vkBindImageMemory(device, depthBufferImages[i], depthBufferMemories[i], 0);
        VkImageViewCreateInfo imageViewCreateInfo{};
        imageViewCreateInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
        imageViewCreateInfo.image = depthBufferImages[i];
        imageViewCreateInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
        imageViewCreateInfo.format = depthBufferFormat;
        ...
        if (vkCreateImageView(device, &imageViewCreateInfo, nullptr, &depthBufferImageViews[i]) != VK_SUCCESS)
        {
...
void Vk_Jello::createImageViews()
{
...
void Vk_Jello::recreateSwapChain()
{
    vkDeviceWaitIdle(device);
...
VkShaderModule Vk_Jello::createShaderModule(const std::vector<char>& code)
{
...
void Vk_Jello::createRenderPass()
{
...
void Vk_Jello::createDescriptorSetLayout()
{
...
void Vk_Jello::createPointGraphicsPipeline()
{
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo,
                                  nullptr,
                                  &m_pointGraphicsPipeline) != VK_SUCCESS)
    {
...
void Vk_Jello::createLineGraphicsPipeline()
{
    auto vertShaderCode = readFile("shaders/vert.spv");
    auto fragShaderCode = readFile("shaders/frag.spv");
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo,
                                  nullptr,
                                  &m_lineGraphicsPipeline) != VK_SUCCESS)
    {
        throw std::runtime_error("failed to create graphics pipeline!");
    }
...
void Vk_Jello::createFramebuffers()
{
    swapChainFramebuffers.resize(swapChainImageViews.size());
    for (size_t i = 0; i < swapChainImageViews.size(); i++)
    {
        VkImageView attachments[] = {swapChainImageViews[i], depthBufferImageViews[i]};
...
void Vk_Jello::createCommandPool()
{
...
void Vk_Jello::initBoundingBoxVertexIndexBuffers()
{
    ...
    m_boundingBoxVertices.resize(boundingBoxVertices.size() *
                                 sizeof(boundingBoxVertices[0]));
    memcpy(m_boundingBoxVertices.data(), boundingBoxVertices.data(),
           boundingBoxVertices.size() * sizeof(boundingBoxVertices[0]));

    m_boundingBoxIndices.resize(boundingBoxIndices.size() *
                                sizeof(boundingBoxIndices[0]));
    memcpy(m_boundingBoxIndices.data(), boundingBoxIndices.data(),
           boundingBoxIndices.size() * sizeof(boundingBoxIndices[0]));
}

void Vk_Jello::createBoundingBoxVertexBuffer()
{
    VkBufferCreateInfo bufferInfo{};
    bufferInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
    bufferInfo.size =
        sizeof(m_boundingBoxVertices[0]) * m_boundingBoxVertices.size();
    bufferInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

    if (vkCreateBuffer(device, &bufferInfo, nullptr,
                       &m_boundingBoxVertexBuffer) != VK_SUCCESS)
    {
        throw std::runtime_error("failed to create vertex buffer!");
    }

    VkMemoryRequirements memRequirements;
    vkGetBufferMemoryRequirements(device, m_boundingBoxVertexBuffer,
                                  &memRequirements);
    ...
    if (vkAllocateMemory(device, &allocInfo, nullptr,
                         &m_boundingBoxVertexBufferMemory) != VK_SUCCESS)
    {
        throw std::runtime_error("failed to allocate vertex buffer memory!");
    }

    vkBindBufferMemory(device, m_boundingBoxVertexBuffer,
                       m_boundingBoxVertexBufferMemory, 0);

    void* data;
    vkMapMemory(device, m_boundingBoxVertexBufferMemory, 0, bufferInfo.size, 0,
                &data);
    memcpy(data, m_boundingBoxVertices.data(), (size_t)bufferInfo.size);
    vkUnmapMemory(device, m_boundingBoxVertexBufferMemory);
}

void Vk_Jello::createBoundingBoxIndexBuffer()
{
    VkDeviceSize bufferSize =
        sizeof(m_boundingBoxIndices[0]) * m_boundingBoxIndices.size();
    ...
    createBuffer(bufferSize,
                 ...
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_boundingBoxIndexBuffer,
                 m_boundingBoxIndexBufferMemory);
...
void Vk_Jello::updateJelloVertexBuffers()
{
...
void Vk_Jello::initJelloVertexIndexBuffers()
{
    const glm::vec3 black = {0.0f, 0.0f, 0.0f};
    std::vector<Vertex> jelloVertices;
...
void Vk_Jello::createJelloVertexBuffer()
{
    ...
    if (vkCreateBuffer(device, &bufferInfo, nullptr, &m_jelloVertexBuffer) !=
        VK_SUCCESS)
    {
        throw std::runtime_error("failed to create vertex buffer!");
    }
    ...
    VkMemoryRequirements memRequirements;
    vkGetBufferMemoryRequirements(device, m_jelloVertexBuffer,
                                  &memRequirements);
    ...
    if (vkAllocateMemory(device, &allocInfo, nullptr,
                         &m_jelloVertexBufferMemory) != VK_SUCCESS)
    {
        throw std::runtime_error("failed to allocate vertex buffer memory!");
    }

    vkBindBufferMemory(device, m_jelloVertexBuffer, m_jelloVertexBufferMemory,
                       0);

    void* data;
    vkMapMemory(device, m_jelloVertexBufferMemory, 0, bufferInfo.size, 0,
                &data);
    memcpy(data, m_jelloVertices.data(), (size_t)bufferInfo.size);
    vkUnmapMemory(device, m_jelloVertexBufferMemory);
}

void Vk_Jello::createJelloIndexBuffer()
{
...
uint32_t Vk_Jello::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
...
void Vk_Jello::createUniformBuffers()
{
...
void Vk_Jello::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
...
void Vk_Jello::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer,
                          VkDeviceSize size)
{
...
void Vk_Jello::createDescriptorPool()
{
...
void Vk_Jello::createCommandBuffers()
{
...
void Vk_Jello::createSyncObjects()
{
...
void Vk_Jello::bindDynamicState(VkCommandBuffer commandBuffer)
{
    VkViewport viewport{};
    viewport.x = 0.0f;
    viewport.y = 0.0f;
...
void Vk_Jello::recordCommandBuffer(VkCommandBuffer commandBuffer,
                                   uint32_t imageIndex)
{
    VkCommandBufferBeginInfo beginInfo{};
    beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
...
VkSurfaceFormatKHR
Vk_Jello::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats)
{
...
VkPresentModeKHR Vk_Jello::chooseSwapPresentMode(
...
VkExtent2D Vk_Jello::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities)
{
...
SwapChainSupportDetails Vk_Jello::querySwapChainSupport(VkPhysicalDevice device)
{
...
bool Vk_Jello::isDeviceSuitable(VkPhysicalDevice device)
{
...
bool Vk_Jello::checkDeviceExtensionSupport(VkPhysicalDevice device)
{
...
QueueFamilyIndices Vk_Jello::findQueueFamilies(VkPhysicalDevice device)
{
...
std::vector<const char*> Vk_Jello::getRequiredExtensions()
{
